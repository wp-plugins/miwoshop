...
class MiwoShopInstall {
	...
	public function createUserTables() {
        ...
        /*$users = MiwoShop::get('db')->run('SELECT u.* FROM #__users AS u, #__user_usergroup_map AS uum WHERE uum.group_id IN (7, 8) AND u.id = uum.user_id AND u.block = 0', 'loadObjectList');
		...
		foreach ($users as $user) {
			...
			$firstname = $name[0];
			$lastname = MiwoShop::get('user')->getLastName($name);
            ...
            $db->run("INSERT IGNORE INTO #__miwoshop_user SET ".
