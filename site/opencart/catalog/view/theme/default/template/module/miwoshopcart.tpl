...
function removeAjaxCart(purduct_key) {
    if (getURLVar('route') == 'checkout/cart' || getURLVar('route') == 'checkout/checkout') {
		...
		jQuery('#cart').load('index.php?option=com_miwoshop&format=raw&tmpl=component&route=module/cart&remove=' + purduct_key + ' #cart > *');

        jQuery.ajax({
			url: 'index.php?option=com_miwoshop&format=raw&tmpl=component&route=module/cart&remove=' + purduct_key + ' #cart > *',
			type: 'post',
